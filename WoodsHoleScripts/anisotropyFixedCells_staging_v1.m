...
addpath('/Users/spira/Desktop/programme/PolyfitnTools/PolyfitnTools')
addpath('/Users/spira/Desktop/programme/kernelDensityEstimator/kde2d')
...
tifFilename = 'cell20_aniso.tif';
tifFilenameMid = 'cell20_zStack_Dapi-1.tif';

I0File = 'cell20_analysis';
fileIdx = 'cell77'
...
% rChromatin2 = 50;
            ...
            %%%%
           imgMid = normalizedImage3D(imgMid);
...
%%%%%% Calculate cleavage furrow diameter
  ...
  for lauf = 1:  AnalysisEnd
                   ...
                   distMinIngressing(lauf) = (dist1*voxelX_mum)/1000;
...
%%%%%% Calculate chromatin chromatin diameter
  ...
  for lauf = 1:  AnalysisEnd
                   ...
                   dist1 = pdist2([cChromatin1(lauf),rChromatin1(lauf)],[cChromatin2(lauf),rChromatin2(lauf)]);
                   
                   distChromatinChromatin(lauf) = (dist1*voxelX_mum)/1000;
  ...
  imshow(imgMid(:,:,3),[],'InitialMagnification',1000)
    ...
    timeToPredict = [distChromatinChromatin distMinIngressing]
...
%%
   %%%% Write file to disk
    save([I0File,'.mat'])
            ...
            saveVariables{7} =  predicted_time;
     ...
     header= ['stagingParameter, intensityCleavageFurrow, intensityPole, anisoFurrow',...
                'anisoPole, anisoRatioFurrowPole, predictedTime']
            
            
            outid = fopen([I0File,'Analysis.csv'], 'w+');

            fprintf(outid, '%s', header);
            fclose(outid);
            dlmwrite ([I0File,'Analysis.csv'],csvData,'roffset',1,'-append')
            
            %%%%% Save information to generate staging
            
              saveVariables = {};

            saveVariables{1} = distChromatinChromatin;
            saveVariables{2} =  distMinIngressing;
            saveVariables{3} =  anisoWindow;
            
             clear csvData;
            csvData=saveVariables;
    
            header= ['chromatinChromatinDist, cleavageFurrowDiameter,Anisotropy']
            
            outid = fopen([I0File,'stagingParameter.csv'], 'w+');

            fprintf(outid, '%s', header);
            fclose(outid);
            dlmwrite ([I0File,'stagingParameter.csv'],csvData,'roffset',1,'-append')
            ...
            outid = fopen([fileIdx,'Analysis.csv'], 'w+');

            fprintf(outid, '%s', header);
            fclose(outid);
            dlmwrite ([fileIdx,'Analysis.csv'],csvData,'roffset',1,'-append')
            ...
            outid = fopen([fileIdx,'stagingParameter.csv'], 'w+');

            fprintf(outid, '%s', header);
            fclose(outid);
            dlmwrite ([fileIdx,'stagingParameter.csv'],csvData,'roffset',1,'-append')
