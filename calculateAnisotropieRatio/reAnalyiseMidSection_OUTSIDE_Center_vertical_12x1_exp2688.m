...
tifFilename= [tifFilename,'_Box12_1'];
...
for lauf = 1:length(xFurrow1)
 ...
 [BW_Box BWHor BWVer,imgMergeSaveVertical]=doDrawBoxAtCoordinates_Ver_12_1(x,y,S1_In,voxelX_mum);
 ...
 [BW_Box BWHor BWVer,imgMergeSaveVertical]=doDrawBoxAtCoordinates_Ver_12_1(x,y,imgMergeSaveVertical,voxelX_mum);
 ...
 [BW_Box BWHor BWVer,imgMergeSaveVertical]=doDrawBoxAtCoordinates_Hor_12_1(x,y,imgMergeSaveVertical,voxelX_mum);
 ...
 [Pole2, Pole2_S1, Pole2_S2] = doCalculateRatioBoundingBox(ratio,BWHor, BW_Box,S1(:,:,lauf), S2(:,:,lauf))
  ...
  saveImg(:,:,:,lauf) = imgMergeSaveVertical;
     ...
     Furrow1Raw_S1_Store(lauf) = (Furrow1_S1+Pole1_S1)/2;
     Furrow1Raw_S2_Store(lauf) = (Furrow1_S2+Pole1_S2)/2;


     Furrow2Raw_S1_Store(lauf) = (Furrow2_S1+Pole2_S1)/2;
     Furrow2Raw_S2_Store(lauf) = (Furrow2_S2+Pole2_S2)/2;
...
%%%%%% This section normalizes Data to pre-Anaphase
     meanFurrow1_S1_preAna = mean(Furrow1Raw_S1_Store(anaOnset-1:anaOnset));
     meanFurrow1_S2_preAna = mean(Furrow1Raw_S2_Store(anaOnset-1:anaOnset));
    
     meanFurrow2_S1_preAna = mean(Furrow2Raw_S1_Store(anaOnset-1:anaOnset));
     meanFurrow2_S2_preAna = mean(Furrow2Raw_S2_Store(anaOnset-1:anaOnset));
     
     
     meanPole1_S1_preAna = mean(Pole1Raw_S1_Store(anaOnset-1:anaOnset));
     meanPole1_S2_preAna = mean(Pole1Raw_S2_Store(anaOnset-1:anaOnset));
     
     meanPole2_S1_preAna = mean(Pole2Raw_S1_Store(anaOnset-1:anaOnset));
     meanPole2_S2_preAna = mean(Pole2Raw_S2_Store(anaOnset-1:anaOnset));
...
SUMRatio = (ratioFurrowStore+ratioPoleStore)./2
   timeSUMRatio = SUMRatio'
   timeSUMRatio = timeSUMRatio
    timeVec = timeVec'
 h=figure

 plot(timeVec(1:end-4),  SUMRatio,'-r')
% hold on
 % plot(timeVec(1:end-1),  ratioPoleStore,'-b')

axis([timeVec(1) 500 0.5 1.2])   

yL = get(gca,'YLim');
line([ingressionTime ingressionTime],yL,'Color','r');


legend('ratio at the cleavage furrow', ...
  'ratio at the pole')


xlabel ('Time [s]','FontSize', 16);
ylabel('Ratio [A.U.]','FontSize', 16);
title(['Ratio green/red at the cleavage furrow and pole' tifFilename],'FontSize', 16);
...
h = figure
 ...
 plot(timeVec(1:end-1), (Furrow_S1_Avg + Furrow_S2_Avg)/2,'-r')
 hold on
  plot(timeVec(1:end-1),(Pole_S1_Avg + Pole_S2_Avg)/2,'-b')
...
title(['Intensities of cleavage furrow and pole normalized to anaphase onset' tifFilename],'FontSize', 16);
