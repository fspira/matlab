function [expectedValues,flank1DiffFromExpected,alphaDegNew] =  doCalculateExpectedIntenisties_splineFit5Knots_3720(funCoords,ratioNormFlank1Tmp,fitresult,testImg,maxMean)
 ...
 %imshow(I3,[])
  ...
  if length(cdv2_StoreSub) < zeroLength
      ...
      zeroLength = length(cdv2_StoreSub);
        ...
        expectedValues(subrun) =  maxMean - (feval(fitresult,(alphaDegNew(subrun)-10)));
      %  flank1DiffFromExpected(subrun) =  flankCrop1(subrun) + (maxMean - (feval(fitresult, alphaDegNew(subrun))));
         
        
         
      anisoCorrectionFactor = (maxMean - (feval(fitresult, alphaDegNew(subrun)-10)));
