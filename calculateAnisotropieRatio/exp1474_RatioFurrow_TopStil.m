...
tifFilename = 'cell7_100nm_vertical.lsm';
%tifFilenameMid = 'cell7_conv.tif';

saveFileName = 'cell7_notch_Center_stressFibres';
...
imgOrigGreen = img(:,:,(timeSet*2)+1);
imgOrigGreen = imgOrigGreen{3};

imgOrigRed = img(:,:,(timeSet*2)+1);
imgOrigRed = imgOrigRed{4};
...
shortAxis = round((6 /voxelX_mum)/2);
longAxis = round((8 / voxelX_mum)/2);
...
cd(curdir)
            ...
            saveVariables{1} = analysisFrame;
            saveVariables{2} = averageRatioD(analysisFrame);
            saveVariables{3} = averageRatio(analysisFrame);
            saveVariables{4} = averageRatioDVer(analysisFrame);
            saveVariables{5} = averageRatioVer(analysisFrame);
