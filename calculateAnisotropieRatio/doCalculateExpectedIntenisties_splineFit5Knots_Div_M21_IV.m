...
function [expectedValues,flank1DiffFromExpected,alphaDegNew,cv2_StoreSub,cdv2_StoreSub] =  doCalculateExpectedIntenisties_splineFit5Knots_Div_M21_IV(funCoords,ratioNormFlank1Tmp,fitresult,testImg,maxMean,metaOffset)
...
marker = 1;
 ...
 for loopRun = 1:5
     ...
     for lauf =marker:zeroLength-5+marker
         ...
         testImg(funCoords(2,lauf),funCoords(1,lauf)) = 255;

     end

 BWTest = im2bw(testImg);
 I3 = bwmorph(BWTest,'bridge',4);
%I3 = BWTest;
 %imshow(I3,[])
     ...
     [skelfit, skeldist, rowI, colI, rowseed, colseed, skelI]=splinefitskel_I(I3,knotSize,5);
  ...
  %  plot(cv1_Store(2,:),cv1_Store(1,:),'r')
...
anisoMetaCorr = feval(fitresult,30);
  ...
  if length(cdv2_StoreSub) < zeroLength
      ...
      zeroLength = length(cdv2_StoreSub);
        ...
        expectedValues(subrun) =  maxMean - (feval(fitresult,(alphaDegNew(subrun)) + metaOffset));
      %  flank1DiffFromExpected(subrun) =  flankCrop1(subrun) + (maxMean - (feval(fitresult, alphaDegNew(subrun))));
         
        
         
      anisoCorrectionFactor = (feval(fitresult, (alphaDegNew(subrun))+ metaOffset));
    
   if  flankCrop1(subrun) >= (feval(fitresult, (alphaDegNew(subrun))+ metaOffset))
