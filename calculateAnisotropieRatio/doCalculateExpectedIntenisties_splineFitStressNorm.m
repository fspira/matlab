function [expectedValues,flank1DiffFromExpected,alphaDegNew,stressNormOut] =  doCalculateExpectedIntenisties_splineFitStressNorm(funCoords,ratioNormFlank1Tmp,fitresult,testImg,maxMean)
 ...
 %imshow(I3,[])
  ...
  if length(cdv2_StoreSub) < zeroLength
      ...
      zeroLength = length(cdv2_StoreSub);
       ...
       stressNormOut(subrun) =  (flankCrop1(subrun)-1.4068) + (maxMean - (feval(fitresult, alphaDegNew(subrun))));
