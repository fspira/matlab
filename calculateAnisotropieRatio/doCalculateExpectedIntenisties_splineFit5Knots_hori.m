function [expectedValues,flank1DiffFromExpected,alphaDegNew] =  doCalculateExpectedIntenisties_splineFitStressNorm_hori(funCoords,ratioNormFlank1Tmp,fitresult,testImg,maxMean)
 ...
 %imshow(I3,[])
  ...
  if length(cdv2_StoreSub) < zeroLength
   ...
   if  flankCrop1(subrun) >= (feval(fitresult, alphaDegNew(subrun)))
        ...
        flank1DiffFromExpected(subrun) =  flankCrop1(subrun) + sqrt(anisoCorrectionFactor^2) ;
    else
       flank1DiffFromExpected(subrun) =  flankCrop1(subrun) + sqrt(anisoCorrectionFactor^2) ;
    end
