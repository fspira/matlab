...
function [expectedValues,flank1DiffFromExpected,alphaDegNew,cv2_StoreSub,cdv2_StoreSub] =  doCalculateExpectedIntenisties_splineFit5Knots_Div_M21_V(funCoords,ratioNormFlank1Tmp,fitresult,testImg,maxMean,metaOffset)
 ...
 %imshow(I3,[])
  ...
  % v = sqrt((cdv2_StoreSub(1,subrun))^2 + cdv2_StoreSub(2,subrun)^2);
   ...
   % IntenistyFlank1(subrun) = greenOverlayStoreNorm(round(cv(1,subrun)),round(cv(2,subrun)))
    rTmp = mod(atan2(cdv2_StoreSub(1,subrun),cdv2_StoreSub(2,subrun)),2.*pi);
    %if rTmp < 0
