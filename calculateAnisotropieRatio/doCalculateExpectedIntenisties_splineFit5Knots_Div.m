function [expectedValues,flank1DiffFromExpected,alphaDegNew] =  doCalculateExpectedIntenisties_splineFit5Knots_Div(funCoords,ratioNormFlank1Tmp,fitresult,testImg,maxMean)
...
anisoMetaCorr = feval(fitresult,45);
  ...
  if length(cdv2_StoreSub) < zeroLength
      ...
      anisoCorrectionFactor = (feval(fitresult, alphaDegNew(subrun)))
    
   if  flankCrop1(subrun) >= (feval(fitresult, alphaDegNew(subrun)))
  
 
    
        flank1DiffFromExpected(subrun) =  flankCrop1(subrun) ./ (anisoCorrectionFactor./anisoMetaCorr);
    else
       flank1DiffFromExpected(subrun) =  flankCrop1(subrun) ./ (anisoCorrectionFactor ./anisoMetaCorr) ;
    end
