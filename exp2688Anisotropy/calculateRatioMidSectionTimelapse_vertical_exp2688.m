...
tifFilename = '3188_None_Mark0001_Tl30s_Movie023.tif';

testImage = tiffread30('polarisation_TR1_2_W0001_P0006_T0001.lsm');
voxelX = getfield(testImage,'lsm','VoxelSizeX');
voxelX_mum = voxelX*1000000;
%tifFilenameMid = 'cell6_conv.tif';

saveFileName = 'cell5_vertical';
...
%%%%% multi file tiff
planeSelector = 1;
imgData = cat(3,imgOrig.data);
   
blueImg =  imgData(:,:,1:3:end);
greenImg = imgData(:,:,2:3:end);
redImg = imgData(:,:,3:3:end);



greenImg = double(greenImg);
redImg = double(redImg);
...
tiffwrite_mat(greenImg,'greenImg.tif')

tiffwrite_mat(redImg,'redImg.tif')


tiffwrite_mat(blueImg,'blueImg.tif')
...
for lauf =1 :p
    ...
    blueStackRGB(:,:,:,lauf) = ind2rgb(normalizedImage(blueImg(:,:,lauf)),blue);
     
    imgMerge(:,:,:,lauf) = greenStackRGB(:,:,:,lauf) + redStackRGB(:,:,:,lauf)+blueStackRGB(:,:,:,lauf);
end


tiffwrite_RBG(imgMerge,'imgMerge.tif')
...
plot(timeVec,furrowRatio)
hold on
plot(timeVec,poleRatio,'r')
...
print(h,'-dpdf', [tifFilename,'_Box_12_1_ratioFurrowPole.pdf']);%tifCurvetifFilename);


h=figure

plot(timeVec,Furrow_S1_RawMeanStore)
hold on
plot(timeVec,Pole1Raw_S1_Store,'r')

legend('Intenisty cleavage furrow', ...
  'Intensity pole')


xlabel ('Time [s]','FontSize', 16);
ylabel('Intensity [A.U.]','FontSize', 16);
title(['Intensities at the cleavage furrow and pole' tifFilename],'FontSize', 16);

print(h,'-dpdf', [tifFilename,'_Box_12_1_IntensityFurrowPole.pdf']);%tifCurvetifFilename);
...
xlabel ('Time [s]','FontSize', 16);
ylabel('Midzone dimaeter [µm]','FontSize', 16);
title(['Midzone/cleavage furrow diameter ' tifFilename],'FontSize', 16);

print(h,'-dpdf', [tifFilename,'_CleavageFurrowDiameter.pdf']);%tifCurvetifFilename);
